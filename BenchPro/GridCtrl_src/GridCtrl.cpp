...
BOOL CGridCtrl::SwapRow(int iRowSource,int iRowDest)
{
    m_bSwapRow = TRUE;
    CString strSource,strDest;
    UINT nFormatSource, nFormatDest;
    for (int iCol = 0; iCol < GetColumnCount(); iCol++)
    {
        strSource =GetItemText(iRowSource, iCol);
        strDest = GetItemText(iRowDest, iCol);
        SetItemText(iRowSource, iCol, strDest);
        SetItemText(iRowDest, iCol, strSource);

        nFormatSource = GetItemFormat(iRowSource, iCol);
        nFormatDest = GetItemFormat(iRowDest, iCol);
        SetItemFormat(iRowSource, iCol, nFormatDest);
        SetItemFormat(iRowDest, iCol, nFormatSource);
    }
    Refresh();
    return TRUE;
}
                       ...
                       CPoint /* point */)
{
	...
	else
	{
		if(m_bDragRowMode)
		{
            ...
            {
                // reoder normal
                CCellRange mCCellRange = GetSelectedCellRange();
                CString strMess;
                strMess.Format(_T("%d [%d] to %d [%d] with range [%d] to [%d]"), m_CurRow,m_CurCol, m_LastDragOverCell.row, m_LastDragOverCell.col, mCCellRange.GetMinRow(), mCCellRange.GetMaxRow());
                AfxMessageBox(strMess);                
                if (m_LeftClickDownCell.col == GetFixedColumnCount() && m_LastDragOverCell.col == m_LeftClickDownCell.col)
                {   
                   /* bool isSwap=false;
                    int iCount = 0;
                    for (int iRow = mCCellRange.GetMinRow(); iRow <= mCCellRange.GetMaxRow(); iRow++)
                    {
                        if(IsCellSelected(iRow, GetFixedColumnCount()))
                            isSwap=SwapRow(iRow, m_LastDragOverCell.row+ iCount);
                        iCount++;
                    }
                    return isSwap;*/
                    if (IsCellSelected(m_CurRow, GetFixedColumnCount()))
                    {
                        int nRowMove = mCCellRange.GetMaxRow() - mCCellRange.GetMinRow()+1;
                        ReorderRow(m_CurRow, nRowMove, m_LastDragOverCell.row);
                        //ReorderRow(m_CurRow, m_LastDragOverCell.row);
                        return true;
                    }
                }
                else
...
void CGridCtrl::ReorderRow(int From, int To)
{    
    if (From == To)
        return;
    m_bSwapRow = TRUE;
    ResetSelectedRange();
        ...
        CGridCellBase** pRowFrom = new CGridCellBase*[m_nCols];
        for (int iCol = 0; iCol < m_nCols; iCol++)
            pRowFrom[iCol] = GetCell(From, iCol);               
        if (From < To)
        {
            ...
            {               
                    ...
                    {
                        if(iRow != To)
                            SetCell(iRow, iCol, GetCell(iRow + 1, iCol));
                        else                       
                            SetCell(iRow, iCol, pRowFrom[iCol]);                      
                    }               
            }
        }
        else
...
void CGridCtrl::ReorderRow(int From,int cRow, int To)
{
    if (From == To || cRow==0)
        return;
    m_bSwapRow = TRUE;
    ...
    if (From < To)
    {        
        CGridCellBase** pRowFrom = new CGridCellBase * [cRow * m_nCols];
        int nIndex = 0;
        // store temp buffer
        for (int iRow = 0; iRow < cRow; iRow++)
        {
            for (int iCol = 0; iCol < m_nCols; iCol++)
                pRowFrom[nIndex++] = GetCell(From + iRow, iCol);
        }
        int nRowMove = To - From;
        if (To + nRowMove > m_nRows)
        {
            nRowMove = m_nRows - To;
            cRow = nRowMove;
        }

        // move rows [To-From] from From+nRowMove to From
        for (int iRowMove = 0; iRowMove < nRowMove; iRowMove++)
        {
            for (int iCol = 0; iCol < m_nCols; iCol++)            
                    SetCell(From+iRowMove, iCol, GetCell(iRowMove+ cRow+From, iCol));
        }
        // copy temp to To
        for (int iRowMove = 0; iRowMove < cRow; iRowMove++)
        {
            for (int iCol = 0; iCol < m_nCols; iCol++)         
                SetCell(To + iRowMove, iCol, pRowFrom[iRowMove* m_nCols+ iCol]);
        }
        delete[]pRowFrom;
        Refresh();
    }
    ...
    {
        int nRowMove = From-To;
        if (nRowMove > cRow)
            nRowMove = cRow;
        int nBuffRow= From - To;
        CGridCellBase** pRowFrom = new CGridCellBase * [nBuffRow * m_nCols];
        int nIndex = 0;
        for (int iRow = 0; iRow < nBuffRow; iRow++)
        {
            for (int iCol = 0; iCol < m_nCols; iCol++)
                pRowFrom[nIndex++] = GetCell(To + iRow, iCol);
        }

        for (int iRowMove = 0; iRowMove < nRowMove; iRowMove++)
        {
            for (int iCol = 0; iCol < m_nCols; iCol++)
                SetCell(To + iRowMove, iCol, GetCell(From+iRowMove,iCol));
        }
        nIndex = 0;
        for (int iRowMove = To + nRowMove; iRowMove < From+ nRowMove; iRowMove++)
        {
           
                for (int iCol = 0; iCol < m_nCols; iCol++)
                    SetCell(iRowMove, iCol, pRowFrom[m_nCols * nIndex + iCol]);

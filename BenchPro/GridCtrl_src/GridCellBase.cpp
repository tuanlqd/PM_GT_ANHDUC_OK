...
BOOL CGridCellBase::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
    ...
    // draw plan logic
    if (nFormat & (DT_LOGIC_HAS_DIR))
    {
        CString cs = GetText();
        CPen* penLogic = NULL;
        if (nFormat & (DT_LOGIC_DIR_VALUE))
            penLogic = new CPen(PS_DOT, 1, RGB(0, 255, 0));
        else
            penLogic = new CPen(PS_DOT, 1, RGB(255, 0, 0));

        CPen* pOldPen = (CPen*)pDC->SelectObject(penLogic);
        for (unsigned char nLoop = 0; nLoop < 4; nLoop++)
        {
            if (cs == _T("0"))
            {
                POINT p1, p2;
                p1.x = rect.left;
                p1.y = rect.bottom - nLoop;
                p2.x = rect.right;
                p2.y = rect.bottom - nLoop;
                pDC->MoveTo(p1);
                pDC->LineTo(p2);
            }
            else if (cs == _T("1"))
    ...
    else if (nFormat & (DT_LOGIC_HAS_REAL))
    {
        // draw real logic
        CPen* penLogic = NULL;
        if (nFormat & (DT_LOGIC_DIR_VALUE))
            penLogic = new CPen(PS_SOLID, 2, RGB(0, 255, 0));
        else
            penLogic = new CPen(PS_SOLID, 2, RGB(255, 0, 0));
        CPen* pOldPen = (CPen*)pDC->SelectObject(penLogic);
        if (nFormat & (DT_LOGIC_REAL_VALUE))
        {
            POINT p1, p2;
            p1.x = rect.left;
            p1.y = rect.top + 2;

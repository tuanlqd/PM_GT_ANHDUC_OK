...
public:
	...
	void setINIFileName(const CString& strINIFile);
	int setTestCaseFileName(const CString& strTestCaseFile);
	
	// methods to return the lists of section data and section names
	CStringList* getSectionData(const CString& strSection);
	...
	long setKey(const CString& strValue, const CString& strKey, const CString& strSection) const;
	...
	BOOL removeSection(const CString& strSection) const;

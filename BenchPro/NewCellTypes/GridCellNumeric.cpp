...
#include "../GridCtrl_src/InPlaceLogicEdit.h"
    ...
    Flags(dwFlags)
{
    ...
    TCHAR* Iptr = sInitText.GetBuffer(sInitText.GetLength() + 1);
...
//////////////////////////
IMPLEMENT_DYNCREATE(CGridCellLogic, CGridCell)
// Create a control to do the editing
BOOL CGridCellLogic::Edit(int nRow, int nCol, CRect rect, CPoint /* point */, UINT nID, UINT nChar)
{
    m_bEditing = TRUE;

    // CInPlaceEdit auto-deletes itself
    m_pEditWnd = new InPlaceLogicEdit(GetGrid(), rect, /*GetStyle() |*/ ES_NUMBER, nID, nRow, nCol,
        GetText(), nChar);
...
// Cancel the editing.
void CGridCellLogic::EndEdit()
{
    if (m_pEditWnd)
        ((InPlaceLogicEdit*)m_pEditWnd)->EndEdit();
}
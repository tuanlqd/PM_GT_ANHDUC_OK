...
class CGridCellLogic : public CGridCell
{
    DECLARE_DYNCREATE(CGridCellLogic)

public:
    void SetDirection(bool isIn)
    {
        DWORD value = GetFormat();
        value |= (DT_LOGIC_HAS_DIR);
        if (isIn)
            value |= (DT_LOGIC_DIR_VALUE);
        else
            value &= ~(DT_LOGIC_DIR_VALUE);
        SetFormat(value);
    }
    void SetRealValue(bool isValue)
    {
        DWORD value = GetFormat();
        value |= (DT_LOGIC_HAS_REAL);
        if (isValue)
            value |= (DT_LOGIC_REAL_VALUE);
        else
            value &= ~(DT_LOGIC_REAL_VALUE);
        SetFormat(value);
    }
